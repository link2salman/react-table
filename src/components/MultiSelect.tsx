...
const MultiSelect = ({ Options, searchable = false, values, setValues }: SelectProps) => {
    ...
    const selectHandler = (e: any) => {
        ...
        if (query) return setQuery({ label: "", value: "" })
    }
    ...
    const removeSelectedOption = (val: any) => setValues(values.filter(value => value != val))
    ...
    const selected = values.flatMap(val => Options.filter(item => item.value === val))
    const selectedClasses = `selected ${selected.length == 1 ? "first-selected-element" : ""}`
    const selectedOptions = selected.map((item, idx) => <span className={selectedClasses} key={`${idx}${item?.label}`} onClick={() => removeSelectedOption(item.value)}>{item.label}</span>)

    const queriedOptions = Options.filter(item => item.label.toLowerCase().includes(query.label.toLowerCase()))
    const optionsList = queriedOptions.map((opt, idx) => <label className='options' htmlFor={opt?.value + idx} key={`${idx}${opt?.value}`} >
        <input type="checkbox" className='checkbox-option-input' id={opt?.value + idx} checked={values.includes(opt?.value)} onChange={() => selectHandler({ target: { label: opt.label, value: opt.value } })} />
        {opt?.label}</label>)

    if (!query.label || ("all").includes(query.label.toLowerCase())) {
        optionsList.unshift(<label className='options' htmlFor={'all'} key={`all 001`}>
    ...
    return (
        <div className='select-container' ref={selectContainerRef} >
            <div className='selected-options' onClick={onFocusOpenCloseOptionList}>
                ...
                <span ref={selectedOptionsRef} className='selected-options-list'> {selectedOptions} {wrapCounter ? <span className='wraped-options-count'>{`${wrapCounter} More`}</span> : ""}</span>
                <span className={`selected-placeholder ${selectedOptions && selectedOptions?.length ? "change-position-animation" : ""}`}>Categories</span>
            </div>
